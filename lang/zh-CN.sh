...
# openclaw-update
MSG_CMD_UPDATE_USAGE="用法: openclaw-update [--sandbox] [--force] [--version=<tag>]"
MSG_CMD_UPDATE_DESC="更新 OpenClaw 到当前 wrapper 对齐的版本（其 VERSION 文件）；可用 --version=<tag> 覆盖。"
...
MSG_CMD_UPDATE_VERSION_OPT="  --version=<tag>  安装指定版本（例如 v2026.5.14-beta.1）— 允许 beta/rc/alpha"
MSG_UPDATE_VERSION_USING="  ⚠️  安装指定版本: %s（跳过稳定版本过滤）"
MSG_UPDATE_WRAPPER_ALIGNED="  → 正在安装 OpenClaw %s（与当前 wrapper 对齐）"
MSG_UPDATE_WRAPPER_DOWNGRADE="  ⚠ 拒绝将 OpenClaw 从 %s 降级到 %s。当前 wrapper 对齐的是更旧的 OpenClaw。如需有意降级，请加 --force 重新运行；如要改用 beta 线，请运行：openclaw-selfupdate --pre"
...
# openclaw-selfupdate（更新 openclaw-orbstack 包装脚本本身，按 release 锁定；不动 VM 里的 OpenClaw）
MSG_CMD_SELFUPDATE_USAGE="用法: openclaw-selfupdate [--pre] [--version=<tag>]"
MSG_CMD_SELFUPDATE_DESC="将 openclaw-orbstack 包装脚本（本仓库）更新到按 release 锁定的标签。不会改动 VM 或 OpenClaw 本身 —— 那用 openclaw-update。"
MSG_CMD_SELFUPDATE_PRE_OPT="  --pre            更新到最新标签，包含预发布版本（beta/rc/alpha）"
MSG_CMD_SELFUPDATE_VERSION_OPT="  --version=<tag>  锁定到指定的包装标签（例如 v2026.6.11）— 允许回滚"
MSG_CMD_SELFUPDATE_TIP="提示: 默认更新到最新稳定包装标签。更新 VM 里的 OpenClaw 请用 openclaw-update。"
MSG_CMD_SELFUPDATE_NO_REPO="错误: 未找到 openclaw-orbstack 仓库。"
MSG_CMD_SELFUPDATE_FETCHING="🔄 正在拉取包装 release 标签..."
MSG_CMD_SELFUPDATE_VERSION_USING="  ⚠️  锁定包装到版本: %s"
...
MSG_FINAL_CMD_SELFUPDATE="更新 wrapper 本身 / 选择发布通道"
...
MSG_REFRESH_CMD_SELFUPDATE="  openclaw-selfupdate     更新 Mac 包装脚本（本仓库）"
