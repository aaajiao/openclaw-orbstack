...
MSG_CMD_UPDATE_DESC="Update OpenClaw to the version this wrapper is aligned with (its VERSION file); override with --version=<tag>."
...
MSG_UPDATE_WRAPPER_DOWNGRADE="  ⚠ Refusing to downgrade OpenClaw from %s to %s. The wrapper is aligned with an older OpenClaw. To downgrade on purpose, rerun with --force. To move onto the beta line instead, run: openclaw-selfupdate --pre"
...
# openclaw-selfupdate (updates the openclaw-orbstack WRAPPER, release-pinned; NOT the VM's OpenClaw)
MSG_CMD_SELFUPDATE_USAGE="Usage: openclaw-selfupdate [--pre] [--version=<tag>]"
MSG_CMD_SELFUPDATE_DESC="Update the openclaw-orbstack wrapper (this repo) to a release-pinned tag. Does not touch the VM or OpenClaw itself — use openclaw-update for that."
MSG_CMD_SELFUPDATE_PRE_OPT="  --pre            Update to the newest tag, including pre-releases (beta/rc/alpha)"
MSG_CMD_SELFUPDATE_VERSION_OPT="  --version=<tag>  Pin to a specific wrapper tag (e.g. v2026.6.11) — allows rollback"
MSG_CMD_SELFUPDATE_TIP="Tip: Default updates to the latest stable wrapper tag. Use openclaw-update to update OpenClaw in the VM."
MSG_CMD_SELFUPDATE_NO_REPO="Error: openclaw-orbstack repo not found."
MSG_CMD_SELFUPDATE_FETCHING="🔄 Fetching wrapper release tags..."
MSG_CMD_SELFUPDATE_VERSION_USING="  ⚠️  Pinning wrapper to version: %s"
...
MSG_FINAL_CMD_SELFUPDATE="Update this wrapper / pick release channel"
...
MSG_REFRESH_CMD_SELFUPDATE="  openclaw-selfupdate     Update Mac wrapper (this repo)"
