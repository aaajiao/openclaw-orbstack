...
**Version:** v2026.7.2-beta.2 | **License:** MIT
...
Since 2026-07-14 the updater also **syncs official plugins** after the doctor passes and before the gateway start. The sync is channel-aware (since 2026-07-15): on a **stable** target it runs `openclaw plugins update --all` — the only path that realigns lockstep plugin versions/channels (upstream PR #94225 wires `syncOfficialPluginInstalls` to `--all` only; a bare per-id update honors the stored pin and self-reports "up to date"). On a **prerelease** target (`-beta/-rc/-alpha`) it must NOT run `--all` — official plugin convergence is channel-context-only and downgrades lockstep plugins back to stable (upstream #97680, observed 2026-07-15: codex beta → stable); instead the updater replays doctor's own plugin-version-drift fix commands (`openclaw plugins update @openclaw/<pkg>@<core-version>`) grepped from `.update-doctor.log`, so the convergence target and id→package mapping stay upstream-canonical. Either path is skipped with a warning when the VM's npm is ≥ 12 (upstream #106189: plugin updates break under npm 12 and a failed update can disable healthy plugins); failures are non-fatal (gateway starts with current plugin versions + manual hint). Since 2026-07-17 the updater additionally **verifies after the gateway reports healthy** that the sync actually stuck: any drift fix commands printed by `gateway status --deep` (explicit `@version` targets — channel-correct on both channels) are replayed once, followed by a restart and re-check. This covers syncs that silently failed to land: doctor dying mid-run leaves no drift commands to replay (e.g. upstream #109867), and a `plugins update` against a wedged state DB installs the npm package but never records the new active version. A doctor-stage state-DB migration failure is also surfaced from the log when the gateway won't start.
